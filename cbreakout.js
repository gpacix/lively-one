...
export class BreakoutGame extends Box {
  ...
  createBricks () {
    ...
    for (let r = 0; r < BRICK_ROWS; r++) {
      ...
      for (let c = 0; c < BRICK_COLS; c++) {
        let yoffset = (r == 4 ? c % 2 : 0);
        const brick = new Brick({
          bounds: rect(
            BRICK_SPACING + c * (brickWidth + BRICK_SPACING), // X position
            startY + (r + yoffset) * (BRICK_HEIGHT + BRICK_SPACING), // Y position
            brickWidth,
            BRICK_HEIGHT
          ),
  ...
  handleWallCollisions () {
    ...
    } else if (this.ball.position.x + this.ball.width >= this.gameWidth) { // Right Wall
      this.ball.dx *= -1;
  ...
  onKeyDown (evt) {
    ...
    } else if (evt.key === 'p' || evt.key === 'P') { // Pause/Unpause toggle
      if (this.isGameRunning) {
        this.stopGame();
        this.updateStatus('PAUSED. Press P to resume.', Color.yellow);
      } else if (!this.isGameOver) { // Don't resume if game over
        this.startGame();
        this.updateStatus(''); // Clear the status message
      }
      // If game is over, "P" does nothing
    }
  }
